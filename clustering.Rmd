...
knitr::opts_chunk$set(warning=F, message=F, echo=T, fig.align="center"
                      ,fig.width=10)
...
dat_tmp = dat %>% 
  # formato long 
  pivot_longer(-id_cols, names_to="variable", values_to="value") %>% 
  arrange(variable) %>% 
...
Vamos a **normalizar** estas variables para que estén en **escalas comparables**. Este paso se conoce como _scaling_. Las normalizaciones más comunes son:
...
Analizamos los _perfiles_ de los países en términos de las variables. Este gráfico nos puede servir para definir la noción de **similitud/disimilitud** apropiada para nuestro problema.
...
gdat = dat %>%
  ...
  pivot_longer(
    cols=-id_cols, names_to="variable", values_to="value"
  )
...
También podemos hacer un análisis más detallado de las distancia entre todos los pares de países.
...
Según la documentación de la función `get_clust_tendency()`, un Hopkins más alto indica mayor tendencia al clustering.
En este caso, el valor del estadístico es significativamente superior a 0.5, entonces podemos concluir que los países no están distribuidos uniformemente en el espacio de las variables. Es decir, presentan una **tendencia a agruparse**.

# Clustering jerárquico

Vamos a ejecutar un algoritmo de **clustering aglomerativo** usando _average linkage_.
...
Otra posibilidad es mirar el **silhouette** promedio de todas las observaciones. El silhouette de cada objeto compara la cercanía con los objetos del proprio cluster (**cohesión**) con la distancia a objetos de otros clusters (**separación**). El estadístico varía entre 1 y -1. 
...
Supongamos que definimos $K=15$, sabiendo que muchos de los clusters probablemente tengan tamaño=1 ("outliers").

```{r}
rownames(dat_c) = dat$country
# clustering jerarquico "cortado" en 15
hc = hcut(dat_c, k=15, hc_method="average", hc_metric="manhattan", stand=F)

```

Veamos el gráfico de silhouette para $K=15$:
...
```{r fig.height=18}
fviz_dend(hc, horiz=T, k=15, repel=T) 
...
```{r, fig.height=8}
fviz_dend(hc, type="phylogenic", k=15, repel=T) 
...
gdat = dat_c_hc %>% 
  filter(outlier == 0) %>% 
  pivot_longer(-c(id_cols, cluster), names_to="variable", values_to="value")
