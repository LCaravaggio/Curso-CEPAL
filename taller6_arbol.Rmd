...
En este documento presentaremos algunos ejemplos de uso del algoritmo CART para analizar las relaciones que existen en una muestra entre una variable dependiente (wage) y una serie de variables predictivas.
...
rpart.plot(cart1, extra = 1, nn = T)

```

## Análisis de parámetros del árbol

Como se puede ver, los valores por default de la función no llevar a un sobreajuste exagerado en las particiones del árbol. Estos valores por default se pueden chequear con el argumento de control de la función: 
...
Si flexibilizamos un poco esos parámetros, podemos ver que el árbol se vuelve más profundo, ajustandose mucho a las particularidades de la muestra. En este primer ejemplo, vamos a reducir el parámetro `cp` (complexity parameter).
...
Primero, hacemos el proceso para calcular el ECM en la muestra de validación para un valor puntual del parámetro.
